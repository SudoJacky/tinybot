...
import { buildDesktopFileTaskOperation } from "../tasks/desktopTaskCenterSources";
import type { DesktopTaskSourceOperation } from "../tasks/desktopTaskCenter";
import type { MountedWorkspaceActionsIsland } from "../components/workspace/workspaceActionsIsland";
import type { MountedWorkspaceDetailIsland } from "../components/workspace/workspaceDetailIsland";
import type { MountedWorkspaceEditorIsland } from "../components/workspace/workspaceEditorIsland";
import type { MountedWorkspaceHeaderIsland } from "../components/workspace/workspaceHeaderIsland";
import type { MountedWorkspaceRecentFilesIsland } from "../components/workspace/workspaceRecentFilesIsland";
...
export function installDesktopWorkspaceFileActions({
  ...
  editor?.addEventListener("input", () => {
    updateWorkspaceDraft(editor.value);
  });
  ...
  async function loadWorkspaceFiles(): Promise<void> {
    try {
      ...
      renderWorkspaceState(targetDocument, state, undefined, Boolean(revealWorkspaceFile), Boolean(exportWorkspaceFile), handleWorkspaceAction, updateWorkspaceDraft);
    }
  }
  ...
  async function exportActiveWorkspaceFile(): Promise<void> {
    ...
    renderWorkspaceState(targetDocument, state, undefined, Boolean(revealWorkspaceFile), true, handleWorkspaceAction, updateWorkspaceDraft);
    try {
  ...
  function updateWorkspaceDraft(draft: string): void {
    state = applyDesktopWorkspaceDraft(state, draft);
    renderWorkspaceState(
      targetDocument,
      state,
      undefined,
...
function renderWorkspaceState(
  ...
  const editorPanel = targetDocument.querySelector<HTMLElement>(".desktop-workspace-editor-panel");
  if (editorPanel) {
    mountWorkspaceEditorVueIsland(editorPanel, state, onDraftInput);
  }
...
function mountWorkspaceRecentFilesVueIsland(
  ...
  void import("../components/workspace/workspaceRecentFilesIsland").then(({ mountWorkspaceRecentFilesIsland }) => {
...
function mountWorkspaceDetailVueIsland(host: HTMLElement, state: DesktopWorkspaceFileState): void {
  ...
  void import("../components/workspace/workspaceDetailIsland").then(({ mountWorkspaceDetailIsland }) => {
...
function mountWorkspaceActionsVueIsland(
  ...
  void import("../components/workspace/workspaceActionsIsland").then(({ mountWorkspaceActionsIsland }) => {
...
function mountWorkspaceEditorVueIsland(
  host: HTMLElement,
  state: DesktopWorkspaceFileState,
  onDraftInput: ((draft: string) => void) | undefined,
): void {
  if (!canMountVueIsland(host)) {
    return;
  }
  const mounted = workspaceEditorIslands.get(host);
  if (mounted) {
    mounted.update(state);
    return;
  }
  void import("../components/workspace/workspaceEditorIsland").then(({ mountWorkspaceEditorIsland }) => {
    workspaceEditorIslands.set(host, mountWorkspaceEditorIsland(host, {
      state,
      onDraftInput,
    }));
  }).catch(() => {
...
function mountWorkspaceHeaderVueIsland(host: HTMLElement, state: DesktopWorkspaceFileState): void {
  ...
  void import("../components/workspace/workspaceHeaderIsland").then(({ mountWorkspaceHeaderIsland }) => {
