...
import { buildDesktopSettingsFormState, buildDesktopSettingsPaneModel } from "../settings/desktopSettingsProviders";
import { buildDesktopTaskCenterItems } from "../tasks/desktopTaskCenter";
import { buildDesktopToolsSkillsPaneModel } from "../tools-skills/desktopToolsSkills";
...
describe("desktop workbench shell", () => {
  ...
  test("captures scroll from the rendered conversation timeline instead of the display-contents shell", () => {
    const targetDocument = new FakeDocument();
    const thread = targetDocument.createElement("section");
    thread.className = "desktop-conversation-thread";
    const timeline = targetDocument.createElement("div");
    timeline.className = "desktop-conversation-timeline";
    Object.defineProperty(thread, "scrollTop", { configurable: true, value: 0, writable: true });
    Object.defineProperty(thread, "scrollHeight", { configurable: true, value: 0, writable: true });
    Object.defineProperty(thread, "clientHeight", { configurable: true, value: 0, writable: true });
    Object.defineProperty(timeline, "scrollTop", { configurable: true, value: 420, writable: true });
    Object.defineProperty(timeline, "scrollHeight", { configurable: true, value: 1200, writable: true });
    Object.defineProperty(timeline, "clientHeight", { configurable: true, value: 500, writable: true });
    thread.append(timeline);

    expect(captureConversationThreadScroll(thread as unknown as HTMLElement)).toMatchObject({
      bottomOffset: 280,
      scrollTop: 420,
      wasNearBottom: false,
    });
  ...
  test("declares non-overlapping native workbench layout rules for composer, scroll, modules, and inspector", () => {
    ...
    expect(conversationThreadRule).not.toContain("display: contents;");
    expect(styleText).toContain("body.desktop-native-workbench .desktop-conversation-thread > .n-config-provider {\n      display: grid;");
    const conversationBodyLayoutRule = styleText.match(/body\.desktop-native-workbench \.desktop-conversation-body-layout \{([\s\S]*?)\n    \}/)?.[1] ?? "";
    expect(conversationBodyLayoutRule).toContain("display: grid;");
    expect(conversationBodyLayoutRule).toContain("grid-template-rows: minmax(0, 1fr) auto;");
    expect(conversationBodyLayoutRule).toContain("height: 100%;");
    expect(conversationBodyLayoutRule).toContain("overflow: hidden;");
    expect(conversationBodyLayoutRule).not.toContain("display: contents;");
    ...
    expect(conversationLayoutRule).toContain("height: 100%;");
    expect(conversationLayoutRule).toContain("min-height: 0;");
    expect(conversationLayoutRule).toContain("overflow: hidden;");
    expect(conversationLayoutRule).not.toContain("height: auto;");
    expect(conversationLayoutRule).not.toContain("overflow: visible;");
    ...
    const conversationTimelineRule = styleText.match(/body\.desktop-native-workbench \.desktop-conversation-timeline \{([\s\S]*?)\n    \}/)?.[1] ?? "";
    expect(conversationTimelineRule).toContain("display: grid;");
    expect(conversationTimelineRule).toContain("align-content: start;");
    expect(conversationTimelineRule).toContain("gap: 6px;");
    expect(conversationTimelineRule).toContain("min-width: 0;");
    expect(conversationTimelineRule).toContain("height: 100%;");
    expect(conversationTimelineRule).toContain("padding: 18px 0 12px;");
    expect(conversationTimelineRule).toContain("overflow-y: auto;");
    expect(conversationTimelineRule).toContain("overflow-x: hidden;");
    ...
    expect(styleText).not.toContain("padding: 18px 0 var(--desktop-composer-reserve);");
