...
describe("rebuilt chat surface", () => {
  ...
  test("renders the right status rail with artifact and subagent previews", () => {
    const host = document.createElement("section");
    ...
    projection.liveSubagents = [
      {
        ...
        task: "Review layout",
        status: "running",
        latestActivity: "Checking visual hierarchy",
    ...
    const rail = host.querySelector("[data-chat-region='status-rail']");
    expect(rail?.querySelector("[data-chat-rail-section='output']")?.textContent).toContain("Implementation report");
    expect(rail?.querySelector("[data-chat-rail-section='subagents']")?.textContent).toContain("Averroes");
    expect(rail?.querySelector("[data-chat-rail-section='subagents']")?.textContent).toContain("Huygens");
    expect(rail?.querySelectorAll("[data-rail-subagent-id]").length).toBe(2);

    rail?.querySelector<HTMLButtonElement>("[data-rail-subagent-id='delegate-a']")?.click();

    expect(host.querySelector("[data-chat-region='detail-surface']")?.getAttribute("data-detail-kind")).toBe("subagent");
    expect(host.querySelector("[data-chat-region='detail-surface']")?.textContent).toContain("Averroes");
  });
