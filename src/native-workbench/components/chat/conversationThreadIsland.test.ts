...
describe("conversation thread Vue island", () => {
  ...
  test("renders subagent shelf and opens trace inspector", async () => {
    ...
    expect(nativeShelfRule).toContain("grid-row: 2;");
  ...
  test("treats wheel input as user scroll intent before the scroll event fires", async () => {
    const host = document.createElement("section");
    ...
    timeline!.scrollTop = 120;
    timeline!.dispatchEvent(new Event("wheel"));
    await flushDetailPanelOpeningMotion();

    expect(host.querySelector<HTMLElement>(".desktop-conversation-timeline")?.scrollTop).toBe(120);
  });

  test("keeps queued scroll restore after a programmatic restore scroll event", async () => {
    const host = document.createElement("section");
    ...
    Object.defineProperty(timeline, "scrollHeight", { configurable: true, value: 0 });
    Object.defineProperty(timeline, "clientHeight", { configurable: true, value: 0 });
    await nextTick();
    await nextTick();
    timeline!.dispatchEvent(new Event("scroll"));
    Object.defineProperty(timeline, "scrollHeight", { configurable: true, value: 1200 });
  ...
  test("does not fold running assistant drafts into a processed group", async () => {
    const host = document.createElement("section");
    ...
    mountConversationThreadIsland(host, {
      ...
      messages: [
        ...
        {
          author: "Tinybot",
          body: ["planning with the child agent"],
          copyable: false,
          references: [],
          time: "10:31 AM",
          tone: "assistant",
    ...
    expect(host.textContent).toContain("planning with the child agent");
    expect(host.textContent).toContain("waiting for child output");
  });
