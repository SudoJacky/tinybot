...
import { sanitizeTextPreview } from "../../chat/chatRunModel";
import { desktopNaiveThemeOverrides } from "../shell/desktopNaiveTheme";
import { renderAgentUiFormCardChildren } from "../agent-ui/agentUiFormCardIsland";
...
import {
  ...
  normalizeToolStatus,
} from "../tools-skills/toolActivityStatus";
import type { ToolActivityIslandOptions } from "../tools-skills/toolActivityIsland";
...
const CONVERSATION_TIMELINE_SCROLL_RESTORING_ATTR = "data-desktop-scroll-restoring";
...
function restoreConversationTimelineScroll(
  ...
  markConversationTimelineProgrammaticRestore(timeline);
...
function ensureConversationTimelineScrollTracking(timeline: HTMLElement): void {
  ...
  const markScrollIntent = () => incrementConversationTimelineScrollVersion(timeline);
  const markScrollEvent = () => {
    if (timeline.getAttribute(CONVERSATION_TIMELINE_SCROLL_RESTORING_ATTR) === "true") {
      return;
    }
    markScrollIntent();
  };
  timeline.addEventListener("scroll", markScrollEvent);
  timeline.addEventListener("wheel", markScrollIntent, { passive: true });
  timeline.addEventListener("touchmove", markScrollIntent, { passive: true });
  timeline.addEventListener("pointerdown", markScrollIntent, { passive: true });
  timeline.addEventListener("keydown", (event) => {
    if (isConversationTimelineScrollKey(event)) {
      markScrollIntent();
    }
  });
}
...
function incrementConversationTimelineScrollVersion(timeline: HTMLElement): void {
...
function markConversationTimelineProgrammaticRestore(timeline: HTMLElement): void {
  timeline.setAttribute(CONVERSATION_TIMELINE_SCROLL_RESTORING_ATTR, "true");
  const win = timeline.ownerDocument.defaultView;
  const clear = () => {
    timeline.removeAttribute(CONVERSATION_TIMELINE_SCROLL_RESTORING_ATTR);
  };
  if (typeof win?.requestAnimationFrame === "function") {
...
function isConversationTimelineScrollKey(event: KeyboardEvent): boolean {
  return [
    "ArrowDown",
    "ArrowUp",
    "End",
