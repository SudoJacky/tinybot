...
import {
  normalizeThreadEffectiveCapabilities,
...
describe("chat runtime capabilities", () => {
  test("normalizes only the decisions used by Chat", () => {
    expect(normalizeThreadEffectiveCapabilities(backendCapabilities(), "thread-1")).toEqual({
      schemaVersion: "tinybot.effective_capabilities.v2",
      threadId: "thread-1",
      evaluatedTurnId: "turn-1",
      capabilities: {
  ...
  test("rejects mismatched threads and unavailable decisions without reasons", () => {
    expect(() => normalizeThreadEffectiveCapabilities(backendCapabilities(), "thread-other"))
    ...
    expect(() => normalizeThreadEffectiveCapabilities(invalid, "thread-1"))
      .toThrow(/without a reason/i);
  });

  test("creates a fail-closed state while capability truth is unavailable", () => {
    expect(unavailableThreadEffectiveCapabilities("thread-1", "loading", "Loading")).toEqual({
      schemaVersion: "tinybot.effective_capabilities.v2",
      threadId: "thread-1",
      capabilities: {
        agent: {
