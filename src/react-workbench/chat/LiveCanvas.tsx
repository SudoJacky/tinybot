...
import { Bell, Bot, ChevronLeft, ChevronRight, MonitorDot, PanelRightClose, PanelRightOpen, RotateCcw, ShieldCheck, X } from "lucide-react";
...
export function LiveCanvas({
  ...
  return (
    <>
      ...
      <aside aria-label="Live Canvas" className="react-live-canvas tinyos" data-mode={mode} id="tinybot-live-canvas">
      <div
      ...
      <header className="react-live-canvas__header tinyos-system-bar">
        ...
        <div className="tinyos-system-bar__status">
          ...
          {snapshot.dialog || snapshot.notifications.length ? <span className="tinyos-attention" title="TinyOS notifications"><Bell aria-hidden="true" size={13} />{snapshot.dialog ? "Action needed" : snapshot.notifications.length}</span> : null}
        </div>
      ...
      <TinyOsShell
        ...
        snapshot={snapshot}
        layoutMode={tinyOsLayoutModeForWidth(widthPx)}
