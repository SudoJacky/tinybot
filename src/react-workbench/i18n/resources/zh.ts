...
export const zh = {
  common: {
    ...
    automations: {
      ...
      details: "详细设置", runsIn: "运行会话", newChatEachRun: "每次新建会话",
      model: "模型", inheritModel: "应用默认模型",
      execution: "执行方式", trigger: "触发方式", manual: "手动运行",
      saveAndRun: "保存并运行", taskOptions: "{{name}}的更多操作", backToTasks: "返回任务列表",
      search: "搜索定时任务", clearSearch: "清空搜索", filterLabel: "最近运行状态",
      filters: { all: "全部", running: "运行中", completed: "已完成", attention: "需要关注" },
      noMatches: "没有符合搜索或筛选条件的任务。", suggestionsTitle: "试试这些任务",
      suggestions: {
        weekly: { title: "项目周报", description: "回顾近期变更，整理待办事项和下一步计划。", prompt: "查看当前工作区过去一周的变更，总结已完成的工作、尚未解决的问题和下一步建议。将报告保存到 reports/weekly-review.md，并在回复中提供文件链接。" },
        files: { title: "工作区文件梳理", description: "了解项目文件结构，找出可以整理的地方。", prompt: "梳理当前工作区的目录结构，说明主要目录的用途。根据实际证据找出重复或未使用的文件，并提出整理建议，不移动或删除文件。将结果保存到 reports/file-review.md，并在回复中提供文件链接。" },
        todos: { title: "待办事项跟进", description: "找出代码中的待办事项，整理成有优先级的行动清单。", prompt: "查找当前工作区中的 TODO 和 FIXME，核对上下文，按优先级整理可执行的待办事项。将带有源文件引用的清单保存到 reports/open-work.md，并在回复中提供文件链接。" },
      },
      viewReport: "查看报告",
      closePreview: "关闭预览",
      fileChanged: "文件已变化，请重新打开查看最新内容。",
      loadMore: "加载更多",
      openInChat: "请从对应会话中打开此文件，查看完整预览。",
      "intro": "保存工作区任务，选择运行时间和执行方式。",
      "create": "新建自动化",
      "refresh": "刷新",
      "loading": "正在加载自动化…",
      "saved": "已保存任务",
      "empty": "创建第一个任务，例如项目变更报告。",
      "run": "立即运行",
      "edit": "编辑",
      "name": "名称",
      "workspace": "工作区",
      "selectWorkspace": "选择已导入的工作区",
      "missing": "不可用",
      "instructions": "任务指令与输出位置",
      "defaultModel": "每次运行使用应用当前的默认模型及推理设置。",
      "save": "保存",
      "cancel": "取消",
      "delete": "删除任务",
      "deleteHint": "删除任务会保留过去的执行记录、会话和输出文件。",
      "history": "执行历史",
      "allRuns": "全部记录",
      "noRuns": "暂无执行记录。",
      "runDetails": "本次使用的指令与配置",
      "openThread": "打开会话与报告",
      "status": {
            "running": "运行中",
            "waiting": "等待处理",
    ...
    menu: {
      ...
      shortcutHelp: "快捷键", quickStart: "快速开始", reportIssue: "报告问题", tinybotRepo: "Tinybot 仓库",
    },
    shell: {
    ...
    tools: {
      ...
      mcpServersDescription: "Tinybot 当前已知的外部工具 Provider。", mcpEmpty: "没有发现 MCP Server。",
      restartMcp: "重启 MCP Server", restartMcpHint: "重启 MCP Server 以应用已保存的更改", availableTools: "可用工具",
      ...
      mcpForm: {
        ...
        editTitle: "配置 {{name}}", editDescription: "更新 Tinybot 全局配置中的这个 MCP Server。",
        editLabel: "配置 {{name}}", disableLabel: "停用 {{name}}", enableLabel: "启用 {{name}}",
        ...
        bearerTokenConfiguredPlaceholder: "已配置 Token",
        bearerTokenConfiguredHint: "已有 Token；留空会保留原值，输入新 Token 会替换原值。",
    ...
    performanceTrace: {
      ...
      memoryStart: "开始记录内存",
      memoryStop: "停止记录内存",
      memoryRecording: "正在跨页面记录 · {{count}} / 300 条",
      memorySamplingDescription: "手动记录会跨页面继续，每 2 秒采样，达到 300 条后停止 · 当前显示 {{count}} 条",
  ...
  settings: {
    ...
    modules: {
      ...
      labs: { label: "实验室", description: "试用实验性功能" },
    },
    labs: {
      eyebrow: "实验性功能",
      title: "实验室",
      description: "试用仍在验证中的功能。实验功能默认关闭，后续可能调整或移除。",
      loading: "正在加载实验设置…",
      loadFailed: "无法加载实验设置：{{message}}",
      reload: "重新加载设置",
      saving: "正在保存…",
      saved: "已保存，将在新的 Agent 轮次生效。",
      saveFailed: "无法保存实验设置：{{message}}",
      nextTurn: "设置全局生效，在新的 Agent 轮次开始时应用；正在运行的轮次保持原配置。",
      actionFusion: {
        title: "Action Fusion（动作融合）",
        description: "允许模型在提交补丁时附带后续命令，减少修改与验证之间的一次模型往返。",
        limits: "需要启用 Exec 工具。补丁成功后执行命令，命令失败保留修改，长命令可继续等待。若命中命令 Hook，Agent 需分步执行。",
      },
    },
    profile: {
      eyebrow: "用量",
    ...
    app: {
      ...
      composerRichText: {
        label: "Composer 启用富文本",
        description: "在输入框内编辑 Markdown 标题、列表和表格等内容。关闭后使用纯文本编辑。",
      },
  ...
  chat: {
    ...
    quickStart: {
      ...
      taskTitle: "想先试着做点什么？", taskDescription: "选择一个示例，编辑提示词，准备好后再发送。",
      connect: "连接模型", firstTask: "完成一个任务", progress: "快速开始进度",
      dismiss: "关闭快速开始", explore: "先看看", resumeTip: "随时可以通过「帮助 → 快速开始」重新打开。",
      project: "了解一个项目", file: "整理一份文件", question: "直接问个问题",
      ...
      validationHint: "获取模型会保存连接配置并检查模型列表接口。首次对话成功后，才能确认模型可以正常回复；也可以手动填写模型 ID。",
      loading: "正在加载配置…", saving: "正在处理…", retry: "重试",
      discovered: "已获取模型列表，请选择或填写模型 ID。", noModels: "服务商没有返回模型，请填写模型 ID，或检查服务商配置。",
      discoveryUnavailable: "当前环境不支持获取模型列表。", discoveryFailed: "无法获取模型，请检查 API 地址和密钥。",
      saveUnavailable: "当前环境不支持保存默认模型。", modelsLoadFailed: "无法加载模型：{{message}}",
    },
    ...
    shell: {
      ...
      resizeSidebar: "调整会话侧栏宽度", resizeSidebarHint: "向左拖动可收起，保持按住向右拖回可展开。双击恢复默认，方向键微调。", sidebarWidth: "{{width}} 像素",
    ...
    annotation: {
      ...
      unitFor: "{{property}}单位",
      sliderFor: "{{property}}滑条",
      colorFor: "{{property}}颜色选择器",
      alphaFor: "{{property}}透明度",
      linkFor: "联动{{property}}四边",
      invalidValue: "请输入有效的 CSS 值",
      sides: { top: "上", right: "右", bottom: "下", left: "左" },
      editProperties: "修改元素属性",
      commentPlaceholder: "添加批注…",
      cancel: "取消批注",
      livePreview: "即时预览 · 尚未修改源码",
      ...
      selectHint: "单击选择元素，直接拖动截取区域。Esc 取消选择，再按退出。",
      drawHint: "在定格截图上拖动框选，或添加矩形、箭头标记。",
      imageTools: "截图标注工具", crop: "框选", rectangle: "矩形", arrow: "箭头", undo: "撤销标记", fullViewport: "整个视口",
      parents: "选择父级元素", reset: "还原属性", instruction: "修改要求", placeholder: "描述你希望如何修改…",
      previewHint: "属性调整仅作临时预览。添加后会附上截图与修改要求，还原页面后可继续批注其他元素。",
      attach: "添加到输入框", working: "处理中…", elementTitle: "元素批注 · {{tag}}", regionTitle: "区域批注", changeCount: "{{count}} 项属性修改",
      captureFailed: "浏览器未返回可用截图。", importFailed: "截图无法保存为图片附件。",
      properties: { text: "文字", color: "文字颜色", "background-color": "背景色", "font-size": "字号", "font-weight": "字重", width: "宽度", height: "高度", padding: "内边距", margin: "外边距", "border-radius": "圆角", opacity: "透明度" },
    },
    sidecar: {
    ...
    artifactReview: {
      ...
      compare: "对比修改",
      working: "正在加载…",
      pending: "已保存请求前的原始文件。保留或恢复前，修改会持续与此版本比较。",
      accepted: "已保留当前版本。",
      restored: "已恢复原始文件。",
      retry: "重试",
      unchanged: "文件内容与保存的版本完全一致。",
      liveFile: "修改已经写入本地文件。保留当前版本会完成此次检查；恢复会用保存的原始文件替换当前文件。",
      waitForAgent: "请等待 AI 完成后，再保留或恢复文件。",
      accept: "保留当前版本",
      restore: "恢复修改前版本",
      valuesOnly: "仅比较单元格值，不验证公式、样式和图表。恢复时会还原完整的原始文件。",
      addedSheets: "新增工作表：{{names}}",
      removedSheets: "删除工作表：{{names}}",
      cellCount: "{{count}} 个单元格发生变化 · 显示 {{shown}} 个",
      noValueChanges: "文件内容发生了变化，但单元格值没有变化；公式、样式或其他工作簿内容可能已改变。",
      cell: "单元格",
      before: "请求前",
      after: "当前文件",
      previewOnly: "预览无法验证文档的全部内容和格式。恢复时会还原完整的原始文件。",
    ...
    details: {
      ...
      officeSelectTextHint: "选中文字后提出修改",
      officeParagraphSelection: "预览段落 {{position}}",
      officeSlideSelection: "幻灯片 {{position}}",
      officeChangeSlide: "修改第 {{slide}} 页",
      officeContentInstruction: "选中内容的修改要求",
      officeCancelSelection: "取消",
