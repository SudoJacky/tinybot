...
const runChainItems: DesktopRunChainItem[] = [
  ...
  },
  {
    ...
    detailSections: [{ type: "text", label: "Path", text: "src/desktopWorkbenchShell.ts" }],
  },
];
...
describe("inspector region Vue island", () => {
  ...
  test("renders Run Chain inspector when no Work Lens is active", async () => {
    ...
    expect(host.querySelector(".desktop-run-chain-detail")?.textContent).toContain("Path: src/desktopWorkbenchShell.ts");
