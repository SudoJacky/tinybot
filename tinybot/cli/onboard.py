"""Interactive onboarding questionnaire for tinybot."""
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]tinybot[{__version__}][/bold cyan]")
    )
    console.print()
...
def _input_model_with_autocomplete(
    ...
    """Get model input (autocomplete disabled - model database unavailable)."""
...
def _input_context_window_with_recommendation(
    ...
    """Get context window input (recommendation disabled - model database unavailable)."""
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note: Model database is disabled, auto-fill is not available.
    """
    # Check if context_window_tokens field exists
    if not hasattr(model, "context_window_tokens"):
        return

    # Auto-fill is disabled - model database unavailable
    console.print("[dim](i) Auto-fill context window disabled (model database unavailable)[/dim]")
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"tinybot.channels.{name}")
