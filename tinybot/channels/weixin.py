...
from tinybot.config.paths import get_media_dir, get_runtime_subdir
from tinybot.config.schema import Base
from tinybot.utils.helper import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.tinybot/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'tinybot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"tinybot-{uuid.uuid4().hex[:12]}"
