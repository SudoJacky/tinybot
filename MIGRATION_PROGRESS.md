...
- 当前批次：Batch 2：native core/shared/config/AgentRunner 最小闭环已进入 verify；tool runtime 已开始推进并收紧 approval-aware policy；session/turn lifecycle 正在推进，已建立 persisted-message 清洗边界、dedupe/truncate、versioned checkpoint helper、`session.persist_turn` RPC 起点、Rust `session.get_history` legal projection、`agent.done.payload.lifecycle` 可观测元数据，以及 `TurnLifecycle.finalizeTurn()` / checkpoint write-clear 抽象；下一步继续补 restore/resume 的完整 lifecycle abstraction。
