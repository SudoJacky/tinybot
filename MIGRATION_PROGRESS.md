...
- 当前批次：Batch 2：native core/shared/config/AgentRunner 最小闭环已进入 verify；tool runtime 已开始推进并收紧 approval-aware policy；session/turn lifecycle 正在推进，已建立 persisted-message 清洗边界、dedupe/truncate、`session.persist_turn` RPC 起点、Rust `session.get_history` legal projection 和 `agent.done.payload.lifecycle` 可观测元数据，下一步继续补 checkpoint helper/module 抽取与更完整的 lifecycle abstraction。
