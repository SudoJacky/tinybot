...
| 13 | active | [ts_mcp_runtime_migration_design.md](ts_mcp_runtime_migration_design.md) | 接入 MCP 外部动态工具层 | Started MCP runtime migration: Phase 1 config/schema contract normalizes server settings, transport auto-detection, allowlists, wrapped names, and nullable JSON Schema; Phase 2 now has `mcpToolWrapper` with fake-session coverage for wrapped `Tool` metadata, raw tool execution, content formatting, timeout/cancel/failure text, and high-risk `mcp` approval metadata. |
