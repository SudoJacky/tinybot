...
| 13 | active | [ts_mcp_runtime_migration_design.md](ts_mcp_runtime_migration_design.md) | 接入 MCP 外部动态工具层 | Started Phase 1 config/schema contract: TS `mcpConfig` now normalizes snake_case/camelCase server settings, auto-detects stdio/SSE/streamable HTTP transports, preserves empty allowlists, sanitizes wrapped tool names, and `mcpSchema` mirrors Python nullable JSON Schema normalization; `parseTinybotConfig()` now consumes the same MCP normalization. |
