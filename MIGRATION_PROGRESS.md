...
| 13 | active | [ts_mcp_runtime_migration_design.md](ts_mcp_runtime_migration_design.md) | 接入 MCP 外部动态工具层 | Started MCP runtime migration: Phase 1 config/schema contract normalizes server settings, transport auto-detection, allowlists, wrapped names, and nullable JSON Schema; Phase 2 has `mcpToolWrapper`; Phase 3 now has a fake-client `McpRuntimeManager` plus a gated native MCP bridge that discovers native fixture tools through `mcp.list_tools`, registers dynamic `mcp_<server>_<tool>` wrappers before runs, reports skipped/unmatched/failed/collision diagnostics, preserves high-risk approval, forwards approved calls to `mcp.call_tool`, and replaces MCP registrations on reconnect while preserving non-MCP tools. |
