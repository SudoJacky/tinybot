...
- Continued Memory Notes/Profile hook migration: TS-native completed turns now run Python-compatible user profile extraction after persistence and write merged `user_profile` plus duplicate-turn metadata through native session RPC.

- Continued Memory Notes Phase 6: native `memory.migrate_legacy_notes` now converts legacy Markdown memory views into idempotent canonical Memory Notes without rewriting the source Markdown files.

- Continued Memory Notes Phase 6: native `memory.refresh_views` now refreshes managed Memory View sections from canonical `memory/notes.jsonl` while preserving unmanaged Markdown content.

- Continued Memory Notes Phase 7: native `memory.rebuild_index` now exposes the deferred vector-index hook as a stable unavailable no-op payload when no vector backend is installed.
...
Channel Bus row 18 update: TS host-RPC connector bridges now expose native audio transcription as `channel.connector.transcribe_audio`, passing file path/API-key inputs through to the host and preserving explicit Rust unavailable-connector responses until platform connectors are installed.
...
Channel Bus row 18 update: TS `ChannelManager.status()` now carries the shared `MessageBus.stats()` snapshot, so native `channel.status` reports inbound/outbound queue sizes, backlog warning threshold, last warning timestamp, accumulated warnings, and closed state.
...
API Runtime row 19 update: TS-native completed Knowledge upload/rebuild job envelopes now include Python-compatible lifecycle timestamp fields.

API Runtime row 19 update: TS-native OpenAI-compatible chat completions now retry blank provider results once at the facade boundary and return the shared Python-compatible empty-final fallback if the retry is still blank.

## Work Log

| Date | Update |
| --- | --- |
| 2026-06-14 | Continued WebUI/API provider-models parity: native `/api/provider-models` now mirrors Python invalid-body handling and profile-only provider model resolution. |
...
| 2026-06-14 | Continued Memory Notes/Profile hook migration: completed TS-native turns now extract user profile entities with the current provider, merge them like Python, and persist `user_profile` plus duplicate-turn metadata through native session RPC. |
| 2026-06-14 | Continued Memory Notes Phase 6: native `memory.migrate_legacy_notes` now migrates legacy Markdown memory views into idempotent canonical notes while preserving the original Markdown files. |
| 2026-06-14 | Continued Memory Notes Phase 6: native `memory.refresh_views` now regenerates managed Memory View sections from canonical notes while preserving unmanaged Markdown content. |
| 2026-06-14 | Continued Memory Notes Phase 7: native `memory.rebuild_index` now returns a stable unavailable no-op result when no vector memory backend is installed. |
...
- [x] Continue WebUI/API provider-models parity: return Python-compatible invalid-body errors and support profile-only provider model requests through native config resolution.
...
- [x] Continue Memory Notes/Profile hook migration: update session `user_profile` from TS-native completed turns through provider-backed extraction and native session persistence.
...
- [x] Continue MCP runtime lazy-connect parity: reuse active native MCP discovery across repeated agent runs with unchanged MCP config.
