...
4. The desktop window installs the workbench shell without an HTTP bootstrap probe or a local TCP
   port.
5. Use the desktop app through native workbench modules for chat, sessions, approvals, settings, providers, tools, Agent Plugins, browser frames, language toggle, and theme toggle where Rust support exists.
...
Tinybot installs and enables the bundled `create-agent-plugin` and `tinybot-mcp` packages during startup. `create-agent-plugin:migrate-agent-plugin` powers the **Migrate Skill or MCP** flow, while `tinybot-mcp:configure-mcp` teaches an Agent to turn setup instructions or a user-provided link into a global MCP definition and verify the connection. Users can disable these built-in plugins, but cannot uninstall them. A bundled update preserves the current enablement state, and startup does not overwrite a user-managed replacement with the same plugin name.
...
Tinybot supports Agent Plugin MCP servers using `stdio` and `streamable-http`. Unsupported or invalid MCP entries are reported or skipped independently so valid skills and sibling servers remain usable. Plugins remain global, while native turns also discover project-local Skills from `.agents/skills/*/SKILL.md` and `.codex/skills/*/SKILL.md`, plus MCP servers from `mcp.json`, `.mcp.json`, or `.github/mcp.json`. Discovery walks from the nearest Git root to the effective working directory; deeper definitions override same-named outer definitions, and `.agents/skills` wins over a same-scope `.codex/skills` definition with the same name. Tinybot does not scan `.codex` for MCP configuration or load the legacy `<backend-workspace>/skills` directory into native turns.

The **Tools & Plugins** page presents Plugins, Skills, MCP servers, and callable Tools as separate views. Its Skills view scans every existing workspace in `WorkspaceRegistry`, de-duplicates the same Skill file when overlapping registered scopes discover it, and retains separate entries for same-named Skills stored in different workspaces. MCP servers, callable Tools, and Agent Graph entries continue to inherit the active Chat conversation or local draft directory across route navigation and fall back to the configured backend workspace when no workspace-backed conversation is active. Chat's slash menu remains workspace-scoped: it requests the catalog only for the active conversation directory, adds a selected Skill as an atomic removable token inline with the user's editable text, and sends only its activation name. The native Turn assembler then resolves that conversation's project-local catalog and injects the full selected Skill before token estimation and Provider dispatch.

The MCP view can add global STDIO and Streamable HTTP servers through one structured form. STDIO configuration accepts a launch command, ordered arguments, non-sensitive literal environment values, same-name host environment-variable passthrough, and an optional working directory; relative working directories resolve from the active workspace. Streamable HTTP configuration accepts a URL, an optional bearer token stored in Tinybot's local configuration, literal HTTP headers, and headers resolved from environment variables. Tinybot redacts the bearer token from public configuration and diagnostic projections. Saving enables the server and all of its discovered tools, persists it under `tools.mcpServers`, reconciles the shared MCP runtime, and refreshes the catalog. The form does not expose editing existing definitions yet.

Agent Turns can read the same global MCP collection through a credential-redacted `mcp.config.list`, add or replace one typed definition with revision protection through `mcp.config.upsert`, and verify it through `mcp.config.status`. Agent calls accept environment-variable references for credentials but never literal tokens or sensitive headers; direct secret storage remains a private MCP-page action. Existing definitions that contain hidden literal credentials cannot be replaced by the Agent. Newly discovered MCP tools become model-visible on the following Turn because the current Turn keeps a fixed tool catalog.

Imported workspaces are recorded once in `~/.tinybot/workspaces.json`. Chat's folder action, project membership editor, Agent Graph, and Hooks all use this registry instead of rebuilding a catalog from every saved conversation. Tinybot canonicalizes a selected existing directory, removes Windows `\\?\` and `\\?\UNC\` prefixes before storing or displaying it, and supports a separate display name. “Forget workspace” removes only the registry record; it never deletes the folder, and Tinybot requires the workspace to be removed from project groups first. On the first run after upgrading, existing Thread working directories and project memberships are imported once.
