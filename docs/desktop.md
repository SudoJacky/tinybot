...
The MCP view can add a global Streamable HTTP server through a structured form. It accepts the server name and URL, an optional bearer token stored in Tinybot's local configuration, literal HTTP headers, and headers resolved from environment variables. Tinybot redacts the bearer token from public configuration and diagnostic projections. Saving enables the server and all of its discovered tools, persists it under `tools.mcpServers`, reconciles the shared MCP runtime, and refreshes the catalog. The form does not expose stdio creation or editing existing definitions yet.
