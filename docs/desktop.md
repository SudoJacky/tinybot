...
The **Tools & Plugins** page presents Plugins, Skills, MCP servers, and callable Tools as separate views. Its workspace-scoped entries inherit the active Chat conversation or local draft directory across route navigation and fall back to the configured backend workspace when no workspace-backed conversation is active. Chat's slash menu requests the catalog for the active conversation directory; selecting a Skill adds an atomic removable token inline with the user's editable text and sends only its activation name. The native Turn assembler then resolves that conversation's project-local catalog and injects the full selected Skill before token estimation and Provider dispatch.
