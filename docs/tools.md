...
Cowork 支持多种协作机制，但它们共享同一套 session、任务、mailbox 和 shared memory：`hybrid`、`orchestrator`/`supervisor`、`team`、`generator_verifier`、`message_bus`、`shared_state`、`peer_handoff`。默认 `hybrid` 会按任务需要组合使用这些机制。
