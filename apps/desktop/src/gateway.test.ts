...
describe("gateway HTTP client", () => {
  ...
  test("uses Python gateway for auto-run cowork create when the scheduler rollout gate is disabled", async () => {
    ...
    await expect(client.cowork.create({
      goal: "Python auto-run alias wins",
      autoRun: "",
      auto_run: true,
    })).resolves.toEqual({ gateway: true });
  ...
  test("uses Python gateway for Python-truthy auto-run cowork create when the scheduler rollout gate is disabled", async () => {
    const fetchFn = vi.fn(async (url: RequestInfo | URL, _init?: RequestInit) => {
      if (String(url).endsWith("/webui/bootstrap")) {
        return new Response(JSON.stringify({ token: "token-1" }), { status: 200 });
      }
    ...
    await expect(client.cowork.create({
      goal: "Auto-run string scheduler fallback",
      architecture: "team",
      auto_run: "yes",
    })).resolves.toEqual({ gateway: true });
  ...
  test("uses Python gateway for explicitly swarm cowork run when the swarm rollout gate is disabled", async () => {
    const fetchFn = vi.fn(async (url: RequestInfo | URL, _init?: RequestInit) => {
      if (String(url).endsWith("/webui/bootstrap")) {
        return new Response(JSON.stringify({ token: "token-1" }), { status: 200 });
      }
    ...
    await expect(client.cowork.run("cw_1", {
      max_rounds: 2,
      workflow_mode: "swarm",
    })).resolves.toEqual({ gateway: true });
  ...
  test("keeps recipientless non-swarm cowork messages on the mutation rollout gate", async () => {
    const fetchFn = vi.fn(async (url: RequestInfo | URL, _init?: RequestInit) => {
      if (String(url).endsWith("/webui/bootstrap")) {
        return new Response(JSON.stringify({ token: "token-1" }), { status: 200 });
      }
  ...
  test("uses Python truthiness when selecting create mode for swarm rollout gates", async () => {
    const fetchFn = vi.fn(async (url: RequestInfo | URL, _init?: RequestInit) => {
      if (String(url).endsWith("/webui/bootstrap")) {
        return new Response(JSON.stringify({ token: "token-1" }), { status: 200 });
      }
  ...
  test("keeps non-swarm branch result select requests on the mutation rollout gate", async () => {
    const fetchFn = vi.fn(async (url: RequestInfo | URL, _init?: RequestInit) => {
      if (String(url).endsWith("/webui/bootstrap")) {
        return new Response(JSON.stringify({ token: "token-1" }), { status: 200 });
      }
    ...
    await expect(client.cowork.selectBranchResult("cw_1", "branch 1", { result_id: "result_1", architecture: "team" })).resolves.toEqual({ native: true });

    expect(nativeCowork.route).toHaveBeenCalledWith({
      method: "POST",
      path: "/api/cowork/sessions/cw_1/branches/branch%201/result/select-final",
      body: { result_id: "result_1", architecture: "team" },
    });
    expect(fetchFn).not.toHaveBeenCalled();
  });
