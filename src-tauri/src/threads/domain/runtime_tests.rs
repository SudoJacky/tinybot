...
fn persisted_runtime_event_item_uses_stable_event_identity() {
    let item = runtime_event_item(
        ...
        Some("turn-1:agent-form-resolution:210".to_string()),
        Some(210),
        Some("2100".to_string()),
        "agent.form.resolution".to_string(),
        Some("user".to_string()),
        Some("user".to_string()),
        json!({ "formId": "form-1" }),
    );
    ...
    assert_eq!(
        item.item_id,
        "thread-runtime:thread-1:turn-1:event-id:turn-1:agent-form-resolution:210"
    );
    ...
    assert_eq!(payload["eventId"], "turn-1:agent-form-resolution:210");
    assert_eq!(payload["itemId"], "form-1");
