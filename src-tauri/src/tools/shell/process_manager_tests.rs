...
fn unknown_process_poll_fails_explicitly() {
    ...
    let error = rpc
        .poll(ShellProcessPollParams {
            ...
            owner_id: Some("turn-missing".to_string()),
            cursor: None,
            yield_time_ms: Some(0),
        })
...
fn terminate_owner_stops_every_owned_process() {
    let fixture = ProcessFixture::new();
    let rpc = shell_rpc(&fixture);
    let first = start_blocking_process(&rpc, "turn-cleanup", "tool-cleanup-1");
    let second = start_blocking_process(&rpc, "turn-cleanup", "tool-cleanup-2");
    assert!(first.running);
    assert!(second.running);
    assert_eq!(rpc.active_process_count(), 2);

    let report = rpc.terminate_owner("turn-cleanup");
    ...
    for process_id in [first.process_id, second.process_id] {
        let output = rpc
            .poll(ShellProcessPollParams {
                process_id,
                owner_id: Some("turn-cleanup".to_string()),
                cursor: Some(0),
                yield_time_ms: Some(0),
            })
...
fn concurrent_termination_waits_for_one_verified_exit() {
    let fixture = ProcessFixture::new();
    let rpc = shell_rpc(&fixture);
    let started = start_blocking_process(&rpc, "turn-concurrent-terminate", "tool-terminate");
    ...
    for _ in 0..2 {
        ...
        handles.push(std::thread::spawn(move || {
            ...
            rpc.terminate(ShellProcessIdParams {
                process_id,
                owner_id: Some("turn-concurrent-terminate".to_string()),
            })
        }));
    }
    barrier.wait();
