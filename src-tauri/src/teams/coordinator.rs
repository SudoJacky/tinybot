...
impl ToolContributor for CoordinatorTools {
    ...
    fn contribute(&self) -> Vec<ToolRegistryEntry> {
        let member = json!({"type":"object","properties":{"id":{"type":"string"},"displayName":{"type":"string"},"toolProfile":{"type":"string","enum":["research","execution","review"],"description":"Saved employee tool preset. research: web browsing, file search and patches, no shell/MCP; execution (default): inherit permitted work tools; review: file and Team result reads only. All presets retain Team handoff tools and forbid publish_data_view. Choose execution for command-based verification or calculations."},"instructions":{"type":"string","description":"Employee role, responsibilities and boundaries. Put task-specific deliverables and completion criteria in task instructions."}},"required":["id","displayName","instructions"],"additionalProperties":false});
        ...
        [
            ("team.recruit", "Recruit employees and start DAG tasks in the background. Each task's instructions must define its scope, deliverable, evidence requirements and checkable completion criteria so the employee knows when to stop. For a new run supply goal; to extend an existing run supply runId, new members and new tasks only. Existing member IDs can be assigned new tasks. Dependencies may reference existing tasks. Employees inherit your model; their saved toolProfile narrows your tool settings. All employee outputs are internal handoffs. Only you publish final conclusions and data views. Returns immediately with runId; use team.wait for results.", json!({"runId":{"type":"string"},"goal":{"type":"string"},"maxConcurrency":{"type":"integer","minimum":1,"maximum":8},"members":{"type":"array","items":member},"tasks":{"type":"array","items":task}}), vec!["members","tasks"]),
