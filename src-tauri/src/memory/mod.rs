...
impl WorkerMemoryRpc {
    ...
    pub(crate) fn rebuild_index(&self) -> Result<Value, crate::protocol::WorkerProtocolError> {
        self.require(WorkerCapability::MemoryRead)?;
        Ok(serde_json::json!({
            "available": false,
            "rebuilt": false,
    ...
    pub(crate) fn refresh_views(&self) -> Result<Value, crate::protocol::WorkerProtocolError> {
    ...
    fn extract_dream_notes_from_evidence(
        &self,
        evidence: &[Value],
    ) -> Result<DreamExtractionResult, crate::protocol::WorkerProtocolError> {
        let mut notes = self.read_notes()?;
    ...
    fn extract_dream_notes_from_legacy_history(
        &self,
        history: &[Value],
    ) -> Result<DreamLegacyExtractionResult, crate::protocol::WorkerProtocolError> {
        let mut notes = self.read_notes()?;
        let mut captured_notes = 0usize;
    ...
    ) -> Result<(), crate::protocol::WorkerProtocolError> {
        if self.policy.allows(&capability) {
            return Ok(());
        }
        Err(crate::protocol::WorkerProtocolError::new(
    ...
    fn dream_memory_context(&self) -> Result<Value, crate::protocol::WorkerProtocolError> {
        let notes = self.read_notes()?;
        Ok(serde_json::json!({
    ...
    fn read_memory_text(
        ...
        relative_path: &str,
        default_value: &str,
    ) -> Result<String, crate::protocol::WorkerProtocolError> {
    ...
    fn read_notes(&self) -> Result<Vec<Value>, crate::protocol::WorkerProtocolError> {
        Ok(self
            .read_notes_with_lines()?
            .into_iter()
            .map(|(note, _line)| note)
    ...
    fn read_notes_with_lines(
        &self,
    ) -> Result<Vec<(Value, usize)>, crate::protocol::WorkerProtocolError> {
    ...
    fn find_note_with_line(
        &self,
        note_id: &str,
    ) -> Result<(Value, usize), crate::protocol::WorkerProtocolError> {
    ...
    fn write_notes(&self, notes: &[Value]) -> Result<(), crate::protocol::WorkerProtocolError> {
    ...
    fn read_conversation_evidence_ids(
        &self,
    ) -> Result<std::collections::HashSet<String>, crate::protocol::WorkerProtocolError> {
    ...
    fn next_evidence_cursor(&self) -> Result<usize, crate::protocol::WorkerProtocolError> {
    ...
    fn max_evidence_cursor(&self) -> Result<usize, crate::protocol::WorkerProtocolError> {
...
fn invalid_memory_request(message: impl Into<String>) -> crate::protocol::WorkerProtocolError {
...
fn required_memory_note_id(note_id: &str) -> Result<&str, crate::protocol::WorkerProtocolError> {
...
fn find_note_mut<'a>(
    notes: &'a mut [Value],
    note_id: &str,
) -> Result<&'a mut Value, crate::protocol::WorkerProtocolError> {
    notes
        .iter_mut()
        .find(|note| note.get("id").and_then(Value::as_str) == Some(note_id))
        .ok_or_else(|| invalid_memory_request(format!("Memory Note not found: {note_id}")))
...
fn memory_io_error(error: std::io::Error) -> crate::protocol::WorkerProtocolError {
    crate::protocol::WorkerProtocolError::new(
...
fn memory_storage_error(error: WorkerStorageError) -> crate::protocol::WorkerProtocolError {
...
fn validate_memory_value<'a>(
    ...
    value: &'a str,
    allowed: &[&str],
) -> Result<&'a str, crate::protocol::WorkerProtocolError> {
...
fn validate_optional_memory_value(
    ...
    value: Option<String>,
    allowed: &[&str],
) -> Result<Option<String>, crate::protocol::WorkerProtocolError> {
...
fn validate_memory_score(
    field: &str,
    value: f64,
) -> Result<f64, crate::protocol::WorkerProtocolError> {
