...
use crate::settings_registry::{
    apply_mcp_runtime_statuses, build_settings_snapshot, SettingsSnapshot, SettingsSnapshotInput,
};
...
fn reconcile_mcp_runtime_if_changed(
    ...
    .map_err(|error| format!("failed to reconcile MCP runtime: {}", error.message))
}
