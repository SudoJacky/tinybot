...
async fn run_native_agent_turn_with_instructions_async(
    ...
    for iteration in start_iteration..context.max_iterations {
        ...
        let provider_phase_conflict = match provider_message_phase {
            crate::agent::runtime_protocol::AgentAssistantMessagePhase::FinalAnswer
                if !provider_response.tool_calls.is_empty() =>
            {
                Some("provider emitted final_answer phase together with tool calls")
            }
            crate::agent::runtime_protocol::AgentAssistantMessagePhase::Commentary
                if provider_response.tool_calls.is_empty() =>
            {
            ...
            if provider_streamed_content || !provider_response.final_content.is_empty() {
                state.emit_event(
                    ...
                    serde_json::json!({
                        ...
                        "classificationSource": if provider_message_phase == crate::agent::runtime_protocol::AgentAssistantMessagePhase::Commentary {
                            "provider"
                        } else {
                            "completion_fallback"
                        },
        ...
        state.emit_event(
            ...
            serde_json::json!({
                ...
                "classificationSource": if provider_message_phase == crate::agent::runtime_protocol::AgentAssistantMessagePhase::FinalAnswer {
                    "provider"
                } else {
                    "completion_fallback"
                },
