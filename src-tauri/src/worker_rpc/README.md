...
`worker_rpc` is the versioned method-routing boundary for native backend
services. The module root is [`../worker_rpc.rs`](../worker_rpc.rs); this
directory contains dispatch groups and shared protocol helpers.
...
service. Files such as `worker_workspace.rs`, `worker_shell.rs`,
`worker_thread/`, and `worker_memory.rs` own their domain behavior.
...
- `protocol.rs`: request validation and typed parameter parsing.
